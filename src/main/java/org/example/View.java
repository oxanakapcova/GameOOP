...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (BaseHero human: ClientCode.teamAll) {
            if (human.getCoord()[0] == x && human.getCoord()[1] == y){
                if (human.getHp() == 0) {
                    out = "|" + (AnsiColors.ANSI_RED + human.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                    break;
                }
                if (ClientCode.teamOwn.contains(human)) out = "|" + (AnsiColors.ANSI_GREEN + human.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                if (ClientCode.teamEnemy.contains(human)) out = "|" + (AnsiColors.ANSI_BLUE + human.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
    ...
    public static void view() {
        ...
        System.out.print("|    ");
        System.out.print(ClientCode.teamOwn.get(0));
